function [r,n,N0] = add_noise(s,SNRdB,L)
...
% and adapted to both sdsn and awgn noise
% [r,n,N0]= add_noise(s,SNRdB) adds AWGN noise vector to signal
...
% density N0 of noise added
 ...
 gamma = db2pow(SNRdB); %SNR to linear scale
 ...
 else %computed noise, awgn to x coordinate and sdsn to y coordinate
     n = sqrt(N0/2)*(randn(size(s))+ 1i*(sqrt(imag(s)).*randn(size(s))));
 end
 
 r = s + n; %received signal
 
 if iscolumn(s_temp), r=r.'; 
   end%return r in original format as s
end
